...
logistic.IRLS<- function(X_mx,Y =Y, beta.initial.vec = c(0,0,0), d.stop.th = 1e-6, it.max = 100) {
  #check this matrix:
	#initialize the beta parameter vector at t=0
...
```{r}
Y <- read.table("phenotypes-lab11.tsv", header = T,stringsAsFactors = F)
geno <- read.table("genotypes-lab11.tsv", header = T)
```
...
Similar as with our processing of covariates, we now form a likelihood test statistic and use it within the chi-square distribution to calculate a p-value.
...
```{r}
logistic.IRLS.pval <- function( Xa, Xd,Y, beta.initial.vec = c(0,0,0), d.stop.th = 1e-6, it.max = 100) {
  ...
  X_mx <- cbind(rep(1,nrow(Y)), Xa, Xd)
  gamma_inv <- gamma_inv_calc(X_mx, beta_t)
	h1 <- logistic.IRLS( X_mx, Y=Y, beta.initial.vec = c(0,0,0), d.stop.th = 1e-6, it.max = 100)
...
for(i in 1:ncol(xa_matrix)){
 allPvals.loop[i] <- logistic.IRLS.pval(Xa = xa_matrix[,i], Xd = xd_matrix[,i], Y)
}
...
for(i in 1:ncol(xa_matrix)){
 allPvals.rec[i] <- logistic.IRLS.pval.recursive(Xa = xa_matrix[,i], Xd = xd_matrix[,i], Y)
}
...
allPvals <- apply(rbind(xa_matrix,xd_matrix), 2, function(x) logistic.IRLS.pval(Xa=x[1:nrow(xa_matrix)], Xd=x[(nrow(xa_matrix)+1):length(x)], Y))
```
...
Intersting, it appears that in this case the apply function is slightly slower than or about comparable to the for loop.  This may be because we are only testing a relatively small number of variants.  Typically when the time is less than a second the computational time predictions are rather incacurate.  It is also important to note that proc.time() actually contains 3 times: user, system and elapsed.  The user time is time charged for instructions, the system time is time charged for CPU calls, and elapsed time is the sum of the prior two.  With this framework we could also test whether the recursive or loop functions are faster.
...
# Make sure your length of beta.inits are as long as the number of independent variables = 1 + 2(num of geno variables) + Z (num of covariate variables)
```
...
I have shown through several different methods how to calculate the pvalue of a genetic association using logistic regression.  However no plot have yet been made, and no comparison has been made to linear regression.  Therefore, this exercise consists of plotting the logistic regression pvalues within a QQ-Plot.  On the same plot, or possibly a second plot, the pvalues derived from a linear regression should also be included.  Please finally provide a very brief comparison between the logistic and linear results - are you surprised by the results.  As a guide, the final plot should appear similar to the following:
...
knitr::include_graphics("lab11_QQ.png")

# QQ plot for logistic values
# a produce a QQ plot for the p values in 3a
obs <- sort(-log10(allPvals))
exp <- seq(from = (1 / length(obs)), to = 1, length.out = length(obs))
# - log 10
exp <- sort(-log10(exp))

# QQ plot linear regression
linpvals <- matrix(data = NA, nrow = ncol(X_mx), ncol = ncol(Y))
for(i in 1:ncol(xa_matrix)){
 linpvals[i] <- pval_calculator_lab7(Y, xa_matrix[,i], xd_matrix[,i])
}
obs_lin <- sort(-log10(linpvals))
...
df = data.frame("exp" = exp, "lin" = obs_lin, "log" = obs)

# melt the dataframe
df.m = melt(df, id.var = 1)
colnames(df.m) <- c("exp", "regression", "pval")

# plot out the melted dataframe using ggplot
ggplot(df.m, aes(x = exp, y = pval, colour = regression)) + geom_point() + ylab('Observed pval')

```



The logistic and linear results mostly match and have a characteristically pretty good fit to a line near the lower expected p values then pull away near significant hits. Linear generally trails behind the logistic p- values which makes sense since the p-values are probably more significant for the logistic model because the data follows this type of distribution better.

And to save you some downloading, this is the classic linear regression pval calculator that we have been using since lab 7.



Go Further: how would you encode this if you had covariates?

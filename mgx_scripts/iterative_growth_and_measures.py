...
sys.path.insert(os.path.join(root_path, 'Desktop', 'roeder_lab_projects'))
...
for i in range(0,len(dirs_lib['meshes'])):
    ...
    if intra_measures:
        f.do_intra_measures(dirs_lib['meshes'][i])

    if intra_display:
        f.do_intra_display(dirs_lib['meshes'][i])
...
for i in range(0, len(dirs_lib['meshes'])-1):
    ...
    if inter_measures:
        f.do_inter_measures(dirs_lib['meshes'][i],dirs_lib['meshes'][i+1])

    if inter_display:
        f.do_inter_display(dirs_lib['meshes'][i],dirs_lib['meshes'][i+1])
